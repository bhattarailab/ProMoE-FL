...
class MimicMultiModal(Dataset):
    def __init__(self, data_root, ann_root, view_type="view1", split="train", dset = "mimic-cxr"):
        super().__init__()
        self.dset = dset
        annFile = ann_root + f'/{dset}-{view_type}.pkl'
        with open(annFile, 'rb') as pickle_file:
            loaded_data = pickle.load(pickle_file)

        self.data = loaded_data[split] if split != 'eval' else loaded_data['train']
        ...
        if self.dset == "mimic-cxr":
            self.image_root = data_root + "/files/"
        else:
    ...
    def __getitem__(self, idx):
        ...
        elif self.dset == "iuxray":
            relative_path = data_item['image']
            report = data_item['report']
            cleaned_report = clean_report_mimic_cxr(report)
        elif self.dset == "chexpert":
            relative_path = data_item['image']
            cleaned_report = ""
        elif self.dset == "padchest":
            relative_path = data_item['image_path']
            report = data_item['report']
            cleaned_report = clean_report_mimic_cxr(report=report)
